...
9. **学情分析**（可视化）：**薄弱知识点雷达图**、**错因标签分布环图**、**掌握度横向柱状图**、**成绩趋势折线图**、薄弱知识点标签云。掌握度由**贝叶斯知识追踪（BKT）**估计：掌握度 = 掌握后验 P(掌握|证据流) × 当前记忆保持率 exp(-t/θ)；错题、练习、复习回忆全部作为贝叶斯证据更新状态，**复习间隔按每点记忆稳定性自适应推导**（取代固定间隔），复习支持**「记得 / 忘了」双反馈**并按学科给出复习方式建议。
10. **学习计划**：AI 基于薄弱点、到期复习与近期考试生成 7 天个性化计划（支持引导词，可动态重新生成）。
11. **练习中心**：为薄弱考点生成**变式训练题**（AI 出题按学科策略调整命题风格，几何题输出图形参数由系统 SVG 渲染）；AI 判题或自评，结果作为贝叶斯证据回写掌握度。
12. **学习笔记**（知识库中心 → 学习笔记页签）：**自由录入**或**拍照上传**（视觉模型 OCR 转写、公式转 LaTeX），AI 按科学笔记方法重整为**思维导图 / 康奈尔笔记 / 结构大纲 / 闪卡**（点击翻转自测）四种结构，可**一键并入知识图谱**（层级包含关系）、思维导图笔记可**存为脑图**；学科笔记组织建议按学科策略注入。
13. **学习报告**：各科目考点掌握度、错因×科目分布、成长轨迹图表 + AI 学情总结。
14. **首页倒计时**：支持创建/删除重要日期倒计时，首页轮播展示。
15. **首页学情概览与 AI 加油站**：首页展示各科平均掌握度、错因标签、薄弱知识点与待复习数量；"AI 加油站"**每小时自动生成一次**鼓励语（结合进步与薄弱数据，1 小时内访问复用缓存、不重复生成，可手动"换一句"强制刷新）。
16. **设置**：AI 接入（base_url / key / 对话 / 备用对话 / 重试次数 / 向量 / **视觉**模型）、**自动分析开关**、**允许 AI 直接修改现有子网开关**（开启=注入已有图谱由 AI 决定并入或新建；关闭=每次分析均新建独立子网）；配置写入 `server/.env` 持久化，主模型失败自动切换备用模型。
17. **监控**：`GET /api/monitor` 查看系统运行状态与近期日志。
18. **版本与更新**：内置 GitHub 仓库地址（可在设置修改），定时对比 `.release/.version` 检测更高版本；提供**增量更新**（替换变更/新增文件、删除已移除文件，可预览差异）与**全量更新**（完整包覆盖全部代码）两种可选模式（均需密码验证）。更新由**专用更新模块**接管执行「下载 → 备份 → 替换 → 依赖 → 构建 → 重启 → 自检 → 失败紧急回滚」，前端实时进度、成功后自动刷新页面。自动更新策略支持「仅检测提醒 / 检测并下载 / 直接完成更新」，可设置检查间隔与代理（详见下方「版本与更新（自更新）」）。
19. **账户与安全体验**：登录、当前账户信息、密码修改（显示密码二次确认）、数据管理危险操作二次确认；深色/浅色主题切换、8 种页面切换动画、图标字体本地化。
...
| POST | /api/study/reviews/:nodeId/complete | 完成复习（body.result: recalled 记得 / forgot 忘了，自适应记忆调度） |
...
| GET/POST/DELETE | /api/notes | 学习笔记管理（自由录入/拍照列表/详情/删除） |
| POST | /api/notes/upload | 笔记拍照上传（视觉模型 OCR 转写） |
| POST | /api/notes/:id/analyze | 笔记结构化整理（mindmap/cornell/outline/flashcards，可并入图谱） |
| POST | /api/notes/:id/mindmap | 将思维导图笔记保存为脑图 |
...
- **更新模式**（执行前可在弹窗中选择，默认模式可在设置中保存）：
  - *增量更新*：通过 GitHub tree 的 blob-SHA 与本地逐文件对比，仅下载并替换**变更/新增**的文件，同时**删除**仓库中已移除的文件；应用前可在弹窗内预览完整差异清单（新增/修改/删除分组）。
  - *全量更新*：下载仓库完整包（tarball），**覆盖全部本地代码**，更新后本地代码与仓库完全一致。
  - 两种模式执行前均校验当前账户密码；`server/data` 数据、`.env` 配置、`node_modules`、密钥文件在任何模式下都**不受影响**。
- **专用更新模块**：更新一经确认即由独立进程（`server/scripts/updater-run.mjs`）完全接管，主进程（含 API/页面）不参与执行。流程为：
  `下载代码 → 备份 → 替换/删除 → 版本文件 → 依赖安装（package.json 变化时按需）→ 前端构建 → PM2 重启 → 自检（/api/health 健康检测）`
- **紧急回滚**：构建失败、重启失败或自检超时，更新模块自动恢复备份、重新构建并重启，使服务回到更新前状态；仅保留最近 5 份备份。
- **前端实时反馈**：更新全程以进度条显示阶段与百分比（下载速度、替换、构建、重启、自检、回滚）；检测到更新模块返回**更新成功**后页面**自动刷新**，失败则展示原因（不刷新）。
- **任务感知**：更新期间若存在运行中的任务（如 AI 分析）会等待其结束或失败。
- **自动更新策略**：`仅检测提醒` / `检测并下载` / `直接完成更新` 三档可选（自动更新按设置的默认模式执行）。
...
>
> **v1.4.1**：修复更新**重启阶段卡死**（PM2 重启命令超时兜底，命令未返回不再阻塞/误回滚，交由健康检测判定）；页面自动刷新增加防抖。
>
> **v1.4.2**：彻底修复强制更新**进度条消失/卡住**（更新发起瞬间不再停掉进度轮询，下载阶段起即可看到进度与实时速度）；配套自动化端到端测试。
>
> **v1.4.3**：彻底解决**卡在重启页面**（检测到重启阶段即等 `/health` 恢复后自动刷新，不等更新进程结束）；**统一完整更新**（移除增量对比，自动更新亦走完整包）；移除「查看将更新的文件」「更新方式」等冗余选项。
>
> **v1.4.4**：修复**保存更新设置密码验证失败**（前端密码传参与后端校验取值不一致，现已统一）。
>
> **v1.4.5**：开发者模式优化——开启后自动定位到「开发者选项」；已开启时再次连点触发弹窗提示已开启；修复开启/清空数据时密码校验缺陷。
>
> **v1.4.6**：开发者模式——修复「已开启」弹窗因点击蒙层而一闪而过（改为仅按钮关闭）；「开发者选项」区域已开启时默认展开并自动滚动定位。
>
> **v1.4.7**：开发者模式——修复「开发者选项」栏无法自由伸缩（单向绑定改为 `v-model:IsExpanded`）。
>
> **v1.4.8**：开发者模式——「开发者选项」改为固定于页面右下角的浮动面板，不再占据页面底部、不挤压/上移导航与页眉。
>
> **v1.4.9**：修复「Token 计量」弹窗一闪而过并跳转空白页（趋势图面积路径计算把字符串当数组调用 `.join()` 导致渲染崩溃）；开发者选项改为**全局挂载**，开发者模式开启后切页不再消失、刷新自动恢复；优化连点 5 次开启判定（未开启不再误弹「已开启」提示）。
>
> **v1.5.0**：**重写更新功能**——恢复并完善**增量更新**（替换变更/新增文件并删除仓库已移除文件，应用前可预览差异清单）与**全量更新**（完整包覆盖全部本地代码）两种可选模式；更新确认后由**专用更新模块**完全接管：下载 → 备份 → 替换 → 依赖按需安装 → 构建 → 重启 → **自检**，自检失败**紧急回滚**恢复备份；前端全程实时进度，检测到**更新成功**后自动刷新页面（失败不刷新、展示原因）。
