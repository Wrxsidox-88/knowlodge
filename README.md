...
17. **版本与更新**：内置 GitHub 仓库地址（可在设置修改），定时对比 `.release/.version` 检测更高版本；提供**增量更新**（替换变更/新增文件、删除已移除文件，可预览差异）与**全量更新**（完整包覆盖全部代码）两种可选模式（均需密码验证）。更新由**专用更新模块**接管执行「下载 → 备份 → 替换 → 依赖 → 构建 → 重启 → 自检 → 失败紧急回滚」，前端实时进度、成功后自动刷新页面。自动更新策略支持「仅检测提醒 / 检测并下载 / 直接完成更新」，可设置检查间隔与代理（详见下方「版本与更新（自更新）」）。
