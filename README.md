...
>
> **v1.3.0**：更新过程全程实时显示**下载进度与速度**及解压/替换/构建/重启/健康检测各阶段进度；强制更新不做任何检查、直接完整包替换；健康检测与回滚状态实时可见，不再卡在"正在重启"。
>
> **v1.4.0**：修复更新健康检测端口（不再卡在重启）；更新期间可查看**详细更新日志弹窗**（构建/重启等命令输出）；服务恢复健康后**自动刷新页面**。
