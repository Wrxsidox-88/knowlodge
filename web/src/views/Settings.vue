<template>
  <div class="col-stack settings-page">
    ...
    <!-- 开发者模式开启弹窗：确认同意风险 + 输入密码（开发者选项浮动面板已全局挂载于 App，切页不消失） -->
    ...
    <div v-if="devAlreadyModal" class="modal-mask">
      <div class="modal" style="width: min(400px, 92vw)">
        ...
        <p class="muted" style="font-size: 13px; line-height: 1.7; margin-bottom: 4px">
          开发者模式已开启，开发者选项以浮动面板显示在本页右下角，可直接使用；切换页面后依旧保留。如需关闭请在该面板点击「关闭开发者模式」。
        </p>
        <div style="display: flex; justify-content: flex-end; gap: 10px">
...
// ---- 开发者模式 ----
// 开发者选项浮动面板已全局挂载于 App.vue（devState.enabled 控制），切页不消失。
// 本页仅保留「连点 5 次系统版本行」的开启入口与开启确认弹窗。
  ...
  if (key === 'osVersion') {
    ...
    if (sysClicks.value.length >= 5) {
      ...
      if (devState.enabled) {
        ...
        devModal.value = true; // 未开启：进入开启确认弹窗
      }
    }
  }
}
...
// chartPolyline 返回的是 SVG 点字符串（"x,y x,y ..."），mArea 需在其上闭合底部两角形成面积填充
const mArea = computed(() => {
  const pts = mPoints.value;
  if (!pts) return '';
  return `${pts} 620,178 0,178`;
});
...
async function openMeter() {
  ...
  try {
    ...
    meterMsg.value = e.message || '加载计量数据失败';
  }
}
